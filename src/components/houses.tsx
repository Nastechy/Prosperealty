...
export default function Houses() {
    ...
    const buildings = properties.filter((p) => p.category === "building");
    const list = isMobile ? buildings.slice(0, visibleCount) : buildings;

    return (
        <div className="bg-white min-h-screen">
            <div className="max-w-7xl mx-auto px-4 my-10 md:py-16">
                {buildings.length === 0 ? (
                    <div className="text-center text-gray-600">No building listings yet. Check back soon.</div>
                ) : (
                        ...
                        {list.map((p) => (
                            <motion.div key={p.id} whileHover={{ y: -5, scale: 1.025, boxShadow: "0 8px 32px 0 rgba(15,23,42,0.09)" }} className="bg-white rounded-xl overflow-hidden shadow-lg border border-gray-100 hover:shadow-xl">
                                ...
                                <div className="p-4">
                                    ...
                                    <div className="flex items-center justify-between text-sm text-gray-5 00 mb-3">
                ...
                {isMobile && buildings.length > visibleCount && (
